...
def generate_answer(question, faiss_index, qa_chain, k=7):
    ...
    rel_ctx = retrieve(question, faiss_index, k=k)
 
    answer = qa_chain.predict(context=rel_ctx, question=question)
...
def summarize_video(video_url):
    ...
    """
    global fetch_trans, proc_trans
   
   
    if video_url:
        fetch_trans = get_transcript(video_url)
        proc_trans = process(fetch_trans)
    else:
    ...
    if proc_trans:
        ...
        summary = summary_chain.run({"transcript": proc_trans})
        return summary
    else:
        return "No transcript available. Please fetch the transcript first."
 
 
def answer_question(video_url, usr_q):
    """
    ...
    Args:
        video_url (str): The URL of the YouTube video from which the transcript is to be fetched.
        usr_q (str): The question posed by the user regarding the video.
    ...
    if proc_trans and usr_q:
        chunks = chunk_transcript(proc_trans)
        ...
        answer = generate_answer(usr_q, faiss_index, qa_chain)
        return answer
    else:
...
with gr.Blocks() as interface:
    ...
    url_in = gr.Textbox(label="YouTube Video URL", placeholder="Enter the YouTube Video URL")
   
    sum_out = gr.Textbox(label="Video Summary", lines=5)
    q_in = gr.Textbox(label="Ask a Question About the Video", placeholder="Ask your question")
    ans_out = gr.Textbox(label="Answer to Your Question", lines=5)
 
    sum_btn = gr.Button("Summarize Video")
    q_btn = gr.Button("Ask a Question")
 
    stat_txt = gr.Textbox(label="Transcript Status", interactive=False)
 
    sum_btn.click(summarize_video, inputs=url_in, outputs=sum_out)
    q_btn.click(answer_question, inputs=[url_in, q_in], outputs=ans_out)
